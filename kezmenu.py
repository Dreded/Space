...
class KezMenu(KezMenuEffectAble):
    ...
    def insert(self, index, option):
        ...
        self.options.insert(index, {'label': option[0], 'callable': option[1]})
    ...
    def draw(self, surface):
        ...
        for o in self.options:
            ...
            if i == self.option and self.focus_color:
                clr = o.get('focus_color', self.focus_color)
            else:
                clr = o.get('color', self.color)
            text = o['label']
            ...
            if self.center:
                indent += (self.width - (ren.get_width()))//2
                o['label_rect'] = pygame.Rect((ol+self.x + indent, ot+self.y + offset), (ren.get_width(),ren.get_height()) )
